...
public class RoomGraph : MonoBehaviour
{
    ...
    {
        instance = this;
        Room r0 = Instantiate(room, new Vector3(0, 0, 0), Quaternion.identity).GetComponent<Room>();
        ...
        while (rooms.Count < 12)
        {
            ...
            {
                Room r = Instantiate(room, new Vector3(to.x, 0, to.y), Quaternion.identity).GetComponent<Room>();
                spotsClaimed.Add(to);
                rooms.Add(to, r);
                rooms[to].pos = to;
            }
